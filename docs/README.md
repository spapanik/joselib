...
joselib is class based, does no cryptography of its own — every
primitive comes from [pyca/cryptography] — and is strict by default:
explicit algorithm allowlists, no `none` algorithm, no `RSA1_5`, no
compressed JWE, and canonical, duplicate-free token parsing.
...
jwt = JWT(key, "HS256", audience="my-api")

token = jwt.encode({"sub": "user-42", "aud": "my-api", "exp": int(time.time()) + 300})
claims = jwt.decode(token)
```

Signing (`JWS`), encryption (`JWE`), and key handling (`OctKey`,
`RSAKey`, `ECKey`, `OKPKey`) are documented in the [Documentation].
